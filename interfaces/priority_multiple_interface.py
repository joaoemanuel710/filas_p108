...
from interfaces.formulas_reference import show_formulas_priority_multiple
...
def priority_multiple_servers_interface():
    ...
    show_formulas_priority_multiple()
    ...
    with col1:
        n = st.number_input("Número de Classes de Prioridade", min_value=1, max_value=10, value=3, step=1, key="priority_multi_n")
    with col2:
        mu = st.number_input("Taxa de Serviço (μ)", min_value=0.00000001, value=2.0, step=0.00000001, format="%.8f", key="priority_multi_mu")
    with col3:
        s = st.number_input("Número de Servidores (s)", min_value=2, max_value=10, value=3, step=1, key="priority_multi_servers")
    ...
    if st.button("Calcular Fila de Prioridade (Múltiplos Servidores)", key="priority_multi_calculate"):
        try:
            if rho >= 1:
